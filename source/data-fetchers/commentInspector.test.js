...
test("extract first comment json from a list", () => {
  ...
  expect(extractedCommentData.json.body).toBe(
    "Yeah, lad was riding tackles like it was the 80s out there."
  );
...
test("extract last comment json from a list", () => {
  ...
  expect(extractedCommentData.json.body).toBe(
    "Was an impressive save but yeah isak was out there playing with defenders like Neymar at Santos"
  );
...
test("handle extracting all comments", () => {
  ...
  expect(list.map((ecd) => ecd.json.id)).toEqual([
    "h27p6o1",
    "h27qd3a",
    "h29f5e2",
  ]);
