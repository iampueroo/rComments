...
test("sticked automod executes when it should", () => {
  ...
  expect(stickied_automod_comment.doesApply(responseData, {})).toBe(true);
  responseData.isLastReply = true;
  expect(stickied_automod_comment.doesApply(responseData, {})).toBe(false);
...
test("stickied automod behavior is correct", () => {
  ...
  stickied_automod_comment.execute.call(spy, responseData, {}).then(() => {
