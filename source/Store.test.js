...
test("getting next comment for first reply", () => {
  const store = new Store();
  const requestParameters = store.getNextCommentRequestParameters(
    "url",
    "commentId"
  );
  expect(requestParameters).toEqual({
  ...
  const nextRequestParams = store.getNextCommentRequestParameters(
    "url",
    "commentId"
  );
  expect(nextRequestParams).toEqual({
